pipeline {
	...
	environment {
		...
		IMAGE_IN_NEXUS = "docker-registry.${env.DOMAIN_GTS}/rdbo/tools/${env.SVC_NAME}"
	...
	stages {
		stage('deploy to devtest') {
			...
			environment {
				SECRET_NAME = 'ja-rdbo-evo-ocp4devtest'
				NAMESPACE = 'retail-devtest'
		...
		stage('deploy to certprod') {
			...
			environment {
				SECRET_NAME = 'ja-rdbo-evo-ocp4certprod'
				NAMESPACE = 'retail-certprod'
		...
		stage('deploy to drprod') {
			...
			environment {
				SECRET_NAME = 'ja-rdbo-evo-ocp4drprod'
				NAMESPACE = 'retail-certprod'
				IMAGE = "${env.IMAGE_IN_QUAY}"
				DOMAIN = "${env.DOMAIN_CERTPROD}"
